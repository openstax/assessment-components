...
const ExercisePreview = ({ exercise, selected, showAllFeedback = false, showChosenAnswer = false, showCorrectAnswer = false, labelAnswers = false, showScoring = false, overlayChildren, questionStates, onGradingSave, }) => {
    ...
    return ((0, jsx_runtime_1.jsx)(StyledExercise, { exercise: showAllFeedback ? exercise : hideAnswerFeedback(exercise), className: `preview-card ${selected ? 'is-selected' : ''}`, previewMode: true, showScoring: showScoring, overlayChildren: overlayChildren, ...exercisePreviewProps(exercise), onGradingSave: onGradingSave }));
};
