...
exports.startMathJax = exports.typesetMath = void 0;
const lodash_1 = require("lodash");
const fp_js_1 = require("lodash/fp.js");
const weak_map_1 = __importDefault(require("weak-map"));
...
const findProcessedMath = (root) => Array.from(root.querySelectorAll('.MathJax math'));
const findUnprocessedMath = (root) => {
...
const typesetLatexNodes = (latexNodes, windowImpl) => () => {
    if ((0, fp_js_1.isEmpty)(latexNodes)) {
        return;
    }
    windowImpl.MathJax.Hub.Queue(() => windowImpl.MathJax.Hub.Typeset(latexNodes), markLatexNodesRendered(latexNodes));
};
const typesetMathMLNodes = (root, windowImpl) => () => {
    const mathMLNodes = findUnprocessedMath(root);
    if ((0, fp_js_1.isEmpty)(mathMLNodes)) {
        return;
    }
...
const markLatexNodesRendered = (latexNodes) => () => {
...
function typesetDocument(root, windowImpl) {
    const latexNodes = findLatexNodes(root);
    windowImpl.MathJax.Hub.Queue(typesetLatexNodes(latexNodes, windowImpl), typesetMathMLNodes(root, windowImpl));
}
const resolveOrWait = (root, resolve, remainingTries = 5) => {
    if (remainingTries > 0
...
const typesetDocumentPromise = (root, windowImpl) => new Promise((resolve) => {
...
const getTypesetDocument = (0, fp_js_1.memoize)((root, windowImpl) => {
    ...
    return (0, lodash_1.debounce)(typesetDocumentPromise, 100, {
        leading: true,
        trailing: false,
    }).bind(null, root, windowImpl);
});
getTypesetDocument.cache = new weak_map_1.default();
// typesetMath is the main exported function.
// It's called by components like HTML after they're rendered
const typesetMath = async (root, windowImpl = window) => {
    await startMathJax();
...
const startMathJax = (0, lodash_1.once)((windowImpl = window) => new Promise((resolve) => {
